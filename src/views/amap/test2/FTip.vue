  ...
  methods: {
    show(context) {
      ...
      (async() => {
        ...
        const b = {}
        if (b?.pois instanceof Array && b?.pois[0]?.photos?.length) {
          this.pics = []
...
<template>
  <div ref="myModule" :class="[pos==='bottom'? 'pos-bottom' : 'pos-top', 'tip', 'hidden'] ">
    <div style="display: flex; justify-content: space-between; display: none;">
      ...
      <span onclick="$refs.myModule.classList.add('hidden')">X</span>
    </div>
    <div v-if="cover">
